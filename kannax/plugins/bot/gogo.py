...
from kannax.utils import check_owner, get_response, rand_key

GOGO = "https://gogoanime.so"
GOGO_DB = {}


@kannax.on_cmd("gogo", about={"header": "Gogo Plugin Help"})
async def gogo_h(message: Message):
    if not kannax.has_bot:
        await message.err("You Need to create a bot via Bot Father", del_in=5)
        return
    link_ = "https://t.me/{}?start=inline".format((await kannax.bot.get_me()).username)
...
if kannax.has_bot:
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern="get_eps(.*)"))
    @check_owner
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern=r"get_currentpg(.*)"))
    @check_owner
