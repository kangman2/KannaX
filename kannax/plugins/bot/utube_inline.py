...
PATH = "./kannax/xcache/ytsearch.json"
...
@kannax.on_cmd(
    "iytdl",
    about={
        "header": "ytdl com botões",
        "usage": "{tr}iytdl [URL / Texto] ou [Responda um URL / Texto]",
    },
    check_downpath=True,
)
async def iytdl_inline(message: Message):
    ...
    if message.client.is_bot:
        ...
        if link is None:
            ...
            resp = (search_.result()).get("resultados")
            if len(resp) == 0:
        ...
        x = await kannax.get_inline_bot_results(bot.username, f"ytdl {input_url}")
        await message.delete()
...
if kannax.has_bot:
    ...
    async def ytdl_download_callback(c_q: CallbackQuery):
        ...
        callback_continue = f"Baixando {media_type} Aguarde..."
        callback_continue += f"\n\nFormato : {disp_str}"
        ...
        refresh_vid = await kannax.bot.get_messages(
