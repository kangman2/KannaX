...
from kannax.utils import clean_obj
...
async def start_vc_(message: Message):
    ...
    await kannax.send(
        CreateGroupCall(
            peer=(await kannax.resolve_peer(chat_id)),
...
async def end_vc_(message: Message):
    ...
    if not (
        ...
        return
    await kannax.send(DiscardGroupCall(call=group_call))
    await message.edit(
...
async def inv_vc_(message: Message):
    ...
    if not await vc_member(message, group_call):
        return
    try:
        await kannax.send(InviteToGroupCall(call=group_call, users=peer_list))
    except Forbidden:
...
async def vcinfo_(message: Message):
    if not (group_call := (await get_group_call(message))):
        return
    gc_data = await kannax.send(GetGroupCall(call=group_call))
    gc_info = {}
...
async def vc_title(message: Message):
    ...
    if not (group_call := (await get_group_call(message))):
        return
    if await kannax.send(EditGroupCallTitle(call=group_call, title=title.strip())):
...
async def get_group_call(
    ...
    chat_peer = await kannax.resolve_peer(message.chat.id)
    if isinstance(chat_peer, (InputPeerChannel, InputPeerChat)):
        if isinstance(chat_peer, InputPeerChannel):
            full_chat = (await kannax.send(GetFullChannel(channel=chat_peer))).full_chat
        elif isinstance(chat_peer, InputPeerChat):
            full_chat = (
                await kannax.send(GetFullChat(chat_id=chat_peer.chat_id))
            ).full_chat
...
async def get_peer_list(message: Message, limit: int = 10) -> Optional[List]:
    ...
    user_ids = [
        member.user.id
        async for member in kannax.iter_chat_members(chat_id, limit=100)
        if not (
            member.user.is_bot
...
async def append_peer_user(user_ids: List, limit: int = None) -> Optional[List]:
    ...
    for uid in user_ids:
        try:
            peer_ = await kannax.resolve_peer(uid)
...
async def vc_member(m: Message, gc: InputGroupCall) -> bool:
    gc_info = await kannax.send(GetGroupCall(call=gc))
