...
async def totalwarns(message: Message):
    ...
    async for warn in WARNS_DB.find(
        ...
        u_mention = (await kannax.get_users(warn["by"])).mention
...
if kannax.has_bot:
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern=r"^remove_warn_(.*)$"))
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern=r"^warnmode_type_(.*)$"))
