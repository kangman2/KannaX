...
from kannax.utils import SafeDict, get_file_id, parse_buttons
...
async def get_note(message: Message) -> None:
    ...
    if can_access or is_global:
        ...
        if not message.client.is_bot and kannax.has_bot and Config.INLINE_NOTES:
            bot = await kannax.bot.get_me()
            try:
                x = await kannax.get_inline_bot_results(
...
async def get_inote(note_id: int, chat_id: int, user_id: int):
    message = await kannax.bot.get_messages(Config.LOG_CHANNEL_ID, note_id)
    caption = ""
    if message.caption:
    ...
    if caption:
        ...
        u_dict = await kannax.get_user_dict(user_id)
        u_dict["mention"] = no_mention(u_dict["mention"])
        chat = await kannax.get_chat(chat_id)
        u_dict.update(
            {
