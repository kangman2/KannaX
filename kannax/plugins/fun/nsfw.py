...
async def age_verification(msg):
    ...
    x = await kannax.get_inline_bot_results(bot.username, "age_verification_alert")
    await msg.delete()
...
if kannax.has_bot:
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern=r"^age_verification_true"))
    async def age_verification_true(_, c_q: CallbackQuery):
        ...
        msg = await kannax.bot.get_messages("useless_x", 19)
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern=r"^age_verification_false"))
    async def age_verification_false(_, c_q: CallbackQuery):
        ...
        msg = await kannax.bot.get_messages("useless_x", 20)
        f_id = get_file_id(msg)
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern=r"^chg_of_decision_"))
    async def chg_of_decision_(_, c_q: CallbackQuery):
        ...
        msg = await kannax.bot.get_messages("useless_x", 21)
        f_id = get_file_id(msg)
