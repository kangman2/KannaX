...
async def add_sudo(message: Message):
    ...
    if user["id"] in Config.SUDO_USERS:
        ...
        if user["id"] in Config.OWNER_ID or user["id"] == (await kannax.get_me()).id:
            await message.edit(
...
@kannax.on_cmd("vsudo", about={"header": "view sudo users"}, allow_channels=False)
...
async def add_sudo_cmd(message: Message):
    ...
    if "-all" in message.flags or "-full" in message.flags:
        ...
        tmp_ = []
        for c_d in list(kannax.manager.enabled_commands):
    ...
    if cmd in Config.ALLOWED_COMMANDS:
        ...
        c_d.lstrip(Config.CMD_TRIGGER) for c_d in list(kannax.manager.enabled_commands)
    ):
...
@kannax.on_cmd("vscmd", about={"header": "view sudo cmds"}, allow_channels=False)
