...
from kannax.utils import check_owner, humanbytes

plugin_regex = comp_regex(r"Path[\s:]{1,5}(kannax/plugins/\w+/\w+\.py)")
...
async def see_info(message: Message):
    ...
    cmd_list = list(kannax.manager.commands)
    found = True
    ...
    if plugin_loc == "/xtra":
        extra_plugins = (
            "https://github.com/code-rgb/KannaX-Plugins/blob/master/plugins/"
        )
        ...
        plugin_link = "{}/blob/{}/kannax/plugins{}/{}.py".format(
            Config.UPSTREAM_REPO, branch, plugin_loc, plugin_name
        )
    local_path = f"kannax/plugins{plugin_loc}/{plugin_name}.py"
...
if kannax.has_bot:
    ...
    @kannax.bot.on_callback_query(filters.regex(pattern=r"^plugin_upload$"))
    @check_owner
    async def plugin_upload_(c_q: CallbackQuery):
        if match := plugin_regex.search(c_q.message.text):
            if os.path.exists(plugin_loc := match.group(1)):
                ...
                await kannax.bot.send_document(
