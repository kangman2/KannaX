...
from kannax import logging, Config
...
async def _init(r_c: Union['_client.KannaX', '_client.KannaXBot'],
                r_m: RawMessage) -> None:
    ...
    async with _INIT_LK:
        if _U_ID and _B_ID:
            return
        if isinstance(r_c, _client.KannaX):
            if not _U_ID:
...
async def _raise_func(r_c: Union['_client.KannaX', '_client.KannaXBot'],
...
async def _is_admin(r_c: Union['_client.KannaX', '_client.KannaXBot'],
                    r_m: RawMessage) -> bool:
    if r_m.chat.type in ("private", "bot"):
        return False
...
def _get_chat_member(r_c: Union['_client.KannaX', '_client.KannaXBot'],
...
async def _bot_is_present(r_c: Union['_client.KannaX', '_client.KannaXBot'],
                          r_m: RawMessage) -> bool:
...
async def _both_are_admins(r_c: Union['_client.KannaX', '_client.KannaXBot'],
                           r_m: RawMessage) -> bool:
...
async def _both_have_perm(flt: Union['types.raw.Command', 'types.raw.Filter'],
                          r_c: Union['_client.KannaX', '_client.KannaXBot'],
                          r_m: RawMessage) -> bool:
    if not await _bot_is_present(r_c, r_m):
        return False
    try:
...
class RawDecorator(RawClient):
    ...
    def _build_decorator(self,
        ...
        def decorator(func: _PYROFUNC) -> _PYROFUNC:
            async def template(r_c: Union['_client.KannaX', '_client.KannaXBot'],
                               r_m: RawMessage) -> None:
                    ...
                    async with await _get_lock(str(flt)):
                        ...
                        if cond:
                            if Config.USE_USER_FOR_CLIENT_CHECKS:
                                if isinstance(r_c, _client.KannaXBot):
                                    return
